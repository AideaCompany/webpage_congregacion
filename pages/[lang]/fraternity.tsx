...
export default function index(props: { localization: Localization }) {
  ...
  const getData = async () => {
    const res = (await client.query({ query: gql(getPages), variables: { name: 'where' } })) as { data: { getPages: any } }
  ...
  return (
    <Layout title={props.localization.translations.where}>
      <>
        <> {dataCMS && data && <FraternityScreen mainPhoto={data.mainPhoto.key} dataCMS={dataCMS} />}</>
      </>
    </Layout>
  )
}
